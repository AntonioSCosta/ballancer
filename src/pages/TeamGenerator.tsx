...
const TeamGenerator = () => {
    ...
    const loadPlayers = () => {
      try {
        ...
        const {
          valid,
          invalid
        } = StorageUtils.validatePlayerData(allPlayers);
        if (invalid.length > 0) {
  ...
  const filteredPlayers = players.filter(player => player.name.toLowerCase().includes(search.toLowerCase()) || player.position.toLowerCase().includes(search.toLowerCase()));
  ...
  const handlePopulateDatabase = () => {
    ...
    const newPlayers = samplePlayers.filter(samplePlayer => !existingPlayers.some(existing => existing.name.toLowerCase() === samplePlayer.name.toLowerCase()));
  ...
  if (loading) {
    return <div className="container mx-auto px-4 py-6 bg-gray-50 dark:bg-gray-900 min-h-screen">
    ...
    opacity: 0
      ...
      <div className="space-y-4 pb-32">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <SearchBar value={search} onChange={setSearch} placeholder="Search players..." className="flex-1" />
          <div className="flex gap-2">
              
              {players.length > 0 && <Button onClick={handleClearDatabase} variant="outline" className="shrink-0 h-11 px-3 text-destructive hover:text-destructive">
                  <Trash2 className="w-4 h-4 mr-1" />
                  <span className="hidden sm:inline">Clear All</span>
                  <span className="sm:hidden">Clear</span>
                </Button>}
              <Button onClick={handleCreatePlayer} variant="outline" className="shrink-0 h-11 px-4">
          ...
          {/* Selection Summary */}
          {selectedPlayers.size > 0 && <div className="bg-card border rounded-lg p-3">
              ...
              {positionAnalysis.warnings.length > 0 && <Alert className="mt-2 bg-yellow-50 border-yellow-200 dark:bg-yellow-950/20">
                  <Info className="h-4 w-4 text-yellow-600" />
                  <AlertDescription className="text-sm">
                    <div className="space-y-1">
                      {positionAnalysis.warnings.map((warning, idx) => <div key={idx} className="text-yellow-800 dark:text-yellow-200">
                          {warning}
                        </div>)}
                      {positionAnalysis.suggestions.length > 0 && <div className="text-xs text-yellow-700 dark:text-yellow-300 mt-1">
        ...
        {filteredPlayers.length > 0 ? <div className="grid grid-cols-2 gap-3 md:gap-4">
            {filteredPlayers.map(player => <motion.div key={player.id} layout onClick={() => togglePlayerSelection(player.id)} className={`cursor-pointer transition-all duration-200 touch-manipulation ${selectedPlayers.has(player.id) ? "ring-2 ring-primary rounded-xl transform scale-[1.02] shadow-lg" : "hover:scale-[1.01] active:scale-[0.98]"}`}>
                <PlayerCard player={player} onEdit={() => handleEditPlayer(player.id)} className="h-full bg-card shadow-md border border-border hover:shadow-lg transition-shadow" />
              </motion.div>)}
        ...
        opacity: 1
            ...
            {players.length === 0 ? <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 justify-center">
                    <Button onClick={handlePopulateDatabase} className="text-sm">
                      <Database className="w-4 h-4 mr-2" />
                      Add 22 Sample Players
                    </Button>
                    <Button onClick={handleCreatePlayer} variant="outline" className="text-sm">
                      <UserPlus className="w-4 h-4 mr-2" />
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-background/95 backdrop-blur-sm border-t border-border p-4 z-50">
        <div className="container mx-auto max-w-md">
          <Button onClick={handleGenerateTeams} disabled={selectedPlayers.size < 10} className="w-full shadow-lg h-12 text-base font-medium touch-manipulation" size="lg">
            <Users className="mr-2 h-5 w-5" />
            Generate Teams ({selectedPlayers.size})
            {selectedPlayers.size < 10 && <span className="ml-2 text-xs opacity-75">
